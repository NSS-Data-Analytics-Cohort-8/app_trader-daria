...
-- a. Develop some general recommendations as to the price range, genre, content rating, or anything else for apps that the company should target.
	...
	SELECT primary_genre, COUNT(DISTINCT name)
	FROM app_store_apps
	GROUP BY primary_genre
	
	SELECT genres, COUNT(DISTINCT name)
	FROM play_store_apps
	GROUP BY genres
	...
	SELECT name, ROUND((a.rating + b.rating)/2,1) AS rated, 
		CASE WHEN CAST(a.price AS money) > CAST(b.price AS money) THEN CAST(a.price*10000 AS money)
			...
			END AS app_cost,
		ROUND((a.rating + b.rating)/2,1)*2.25*12 AS app_life_months,
		CAST(((ROUND((a.rating + b.rating)/2,1)*2.25*12)*10000)-(ROUND((a.rating + b.rating)/2,1)*2.25*12)*1000 AS money) AS revenue_lifetime_app,
		CAST(((ROUND((a.rating + b.rating)/2,1)*2.25*12)*10000)-(ROUND((a.rating + b.rating)/2,1)*2.25*12)*1000 AS money) 
		-
		CASE WHEN CAST(a.price AS money) > CAST(b.price AS money) THEN CAST(a.price*10000 AS money)
			WHEN CAST(a.price AS money) < CAST(b.price AS money) THEN CAST(CAST(b.price AS money)*10000 AS money)
			WHEN CAST(a.price AS money) <= '$1.00' AND CAST(b.price AS money) <= '$1.00' THEN '$10,000.00'
...
-- b. Develop a Top 10 List of the apps that App Trader should buy.
	...
	WITH app_cost AS 
		(
		SELECT name, ((a.rating+b.rating)/2)*2.25*12 AS applife_months,
		CASE WHEN CAST(a.price AS money) > CAST(b.price AS money) THEN CAST(a.price*10000 AS money)
			WHEN CAST(a.price AS money) < CAST(b.price AS money) THEN CAST(CAST(b.price AS money)*10000 AS money)
			WHEN CAST(a.price AS money) <= '$1.00' AND CAST(b.price AS money) <= '$1.00' THEN '$10,000.00'
			WHEN CAST(a.price AS money) = CAST(b.price AS money) AND CAST(b.price AS money) > '$1.00' AND CAST(b.price AS money) > '$1.00' THEN CAST(CAST(b.price AS 					money)*10000 AS money)
	...
	SELECT ac.name, (CAST(ac.applife_months*10000 AS money)-CAST(ac.applife_months*1000 AS money)-ac.front_cost) AS app_earnings
	FROM app_cost AS ac
